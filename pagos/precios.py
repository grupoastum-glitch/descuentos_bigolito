"""Escalamiento de precios por tramos + precio fijo de por vida (ver el plan en
tengo-una-idea-de-fizzy-dragonfly.md). Duplicado a propósito en bot/precios.py: son dos procesos
separados (bot/ y pagos/) sobre la misma Postgres, mismo criterio que la duplicación de
CANAL_CHAT_ID (ver pagos/config.py) — acá se duplica lógica/ladder, no un número de plata suelto,
que sigue centralizado en web/data/config.json."""
...
async def resolver_precio(
    ...
    """El precio a cobrar/mostrar a esta persona para este canal: si ya pagó antes acá, el mismo
    ...
    guardado real ocurre en pagos/logica.py cuando el pago se confirma (ver
    db.upsert_suscripcion), usando el monto real que MercadoPago terminó cobrando."""
